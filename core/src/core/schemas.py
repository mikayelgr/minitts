...
class TTSJobResult(BaseModel):
    ...
    job_id: UUID = Field(..., description="Unique identifier for the TTS job")
    state: JobState = Field(..., description="The current state of the TTS job")
...
class TTSWebhookPayload(BaseModel):
    ...
    job_id: UUID = Field(..., description="Unique identifier for the TTS job")
    state: JobState = Field(..., description="The final state of the TTS job")
